...
#define GPIO_SW_DEVID 		XPAR_AXI_GPIO_0_DEVICE_ID
...
XGpio gpio;
u32 direction_mask = 0xFFFFFFFF;
u8 input_value;

//Axi Switch Configuration
XAxis_Switch inputSwitch;
XAxis_Switch outputSwitch;

XAxis_Switch_Config *Config1;
...
void gpio_init(){
	int status = XGpio_Initialize(&gpio, GPIO_SW_DEVID);
	if(status == XST_SUCCESS){
		xil_printf("Device init success\n");
	} else {
		xil_printf("fail\n");
	}
}
...
int initSwitch(){
	Config1 = XAxisScr_LookupConfig(SRC_AXIS_SW_DEVID);
	Config2 = XAxisScr_LookupConfig(DST_AXIS_SW_DEVID);

	if (NULL == Config1 || NULL == Config2) {
		return XST_FAILURE;
	}

	int Status1 = XAxisScr_CfgInitialize(&inputSwitch, Config1,
							Config1->BaseAddress);
	int Status2 = XAxisScr_CfgInitialize(&outputSwitch, Config2,
								Config2->BaseAddress);

	if(Status1 != XST_SUCCESS || Status2 != XST_SUCCESS){
		xil_printf("FAILED INITIALIZING AXI SWITCHES");
		return XST_FAILURE;
	}
}

void sourceSwitch(u8 input){
	XAxisScr_RegUpdateDisable(&inputSwitch);
	XAxisScr_RegUpdateDisable(&outputSwitch);

	XAxisScr_MiPortDisableAll(&inputSwitch);
	XAxisScr_MiPortDisableAll(&outputSwitch);
	...
	XAxisScr_MiPortEnable(&inputSwitch, MiIndexInput, SiIndexInput);
	XAxisScr_MiPortEnable(&outputSwitch, MiIndexOutput, SiIndexOutput);

	XAxisScr_RegUpdateEnable(&inputSwitch);
	XAxisScr_RegUpdateEnable(&outputSwitch);
}
...
int main()
{
	...
	while(1){
		input_value = XGpio_DiscreteRead(&gpio, 1); // Read from channel 1
		//xil_printf("Value read: %d\n", input_value);

		if(input_value < 5){
			sourceSwitch(input_value);
			//xil_printf("AXI Switch Triggered\n");
		    switch (input_value) {
		        case 0:
		            xil_printf("Routing Normal View\n");
		            break;

		        case 1:
		            xil_printf("Routing Black/White Filter\n");
		            break;

		        case 2:
		            xil_printf("Routing Gaussian Blur Filter\n");
		            break;

		        case 3:
		            xil_printf("Routing Invert Filter\n");
		            break;

		        case 4:
		            xil_printf("Routing Edge Detect Filter\n");
		            break;

		        default:
		            xil_printf("Outside Value Range\n");
		            break;
		    }
		}
